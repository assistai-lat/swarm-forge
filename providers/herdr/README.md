...
- **Flutter bajo Smart App Control (Windows 11).** Con Smart App Control activo (`VerifiedAndReputablePolicyState = 1`), Windows bloquea `dartvm.exe`, que el SDK de Dart (probado con Flutter 3.47.3 / Dart 3.13.3) lanza sin firma. Todo comando `flutter` muere con `ProcessStarter::StartForExec failed: Una directiva de Control de aplicaciones bloqueó este archivo`, también dentro de los panes de herdr, así que `worker_mobile` y el `victory-auditor` no pueden verificar nada. En el visor de eventos (`Microsoft-Windows-CodeIntegrity/Operational`) aparecen los eventos 3033, 3077 y 3118. Antes de la fase 2, comprueba que `flutter --version` corre en un pane de herdr. Si falla, verifica esa superficie en otra máquina, en WSL o en CI: Smart App Control no admite excepciones por programa, y desactivarlo es una decisión de seguridad del equipo, no del enjambre.
